...
v_cntrl = iris.load_cube(datadir, iris.AttributeConstraint(STASH='m01s00i003'))
v_cntrl = v_cntrl.extract(latConstraint & lonConstraint)
v_cntrl = v_cntrl.regrid(orogc[0], iris.analysis.Linear())
v_cube = modelLevel2RegularHeight(v_cntrl, targetHeight)
data_lons = orogc.coord(axis='x').points
data_lats = orogc.coord(axis='y').points
#----------------------------------------------------------------------------------------------
print('Saving variable as a NetCDF file ...')
# V component of  wind
file_name2 = 'pre_v_wind_YMD_CYC_dayDVAL.nc'
file_path2 = os.path.join(output_dir, file_name2)
if os.path.exists(file_path2):
    os.remove(file_path2)

root_grp = Dataset(file_path2, 'w', format='NETCDF4')
root_grp.Description = 'NCMRWF Unified Global Model Forecast (Vn11.2)'
root_grp.Conventions = 'CF-1.6'
nxdim = len(data_lons)
nydim = len(data_lats)
tdim = len(v_cube.coord('time').points)
nlev = len(v_cube.coord('height').points)
...
temp = root_grp.createVariable('v', 'f8', ('time', 'lev', 'lat', 'lon',))
temp.long_name = "Meridional wind speed"
temp.units = "m/s"
temp.level = "A1"
lon[:] = v_cube.coord(axis='x').points
lat[:] = v_cube.coord(axis='y').points
lev[:] = v_cube.coord('height').points
